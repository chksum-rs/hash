...
mod tests {
    ...
    fn test_md5() {
        ...
        let digest = new(Algorithm::MD5).digest();
        ...
        assert_eq!(new(Algorithm::MD5).update("data").finalize().digest(), digest);
        assert_eq!(Digest::from(md5::hash(b"data")), digest);

        let digest = new(Algorithm::MD5).update("data").reset().digest();
        assert_eq!(digest.to_hex_lowercase(), "d41d8cd98f00b204e9800998ecf8427e");
        assert_eq!(digest.to_hex_uppercase(), "D41D8CD98F00B204E9800998ECF8427E");

        let digest = new(Algorithm::MD5).update("data").finalize().reset().digest();
        assert_eq!(digest.to_hex_lowercase(), "d41d8cd98f00b204e9800998ecf8427e");
        assert_eq!(digest.to_hex_uppercase(), "D41D8CD98F00B204E9800998ECF8427E");
    }
    ...
    #[test]
    fn test_sha1() {
        assert_eq!(format!("{}", Algorithm::SHA1), "SHA-1");

        let digest = new(Algorithm::SHA1).digest();
        ...
        assert_eq!(verify(Algorithm::SHA1, b"", digest), true);
        assert_eq!(format!("{digest:x}"), digest.to_hex_lowercase());
        assert_eq!(format!("{digest:X}"), digest.to_hex_uppercase());

        let digest = new(Algorithm::SHA1).update("data").digest();
        ...
        assert_eq!(new(Algorithm::SHA1).update("data").finalize().digest(), digest);
        assert_eq!(Digest::from(sha1::hash(b"data")), digest);

        let digest = new(Algorithm::SHA1).update("data").reset().digest();
        assert_eq!(digest.to_hex_lowercase(), "da39a3ee5e6b4b0d3255bfef95601890afd80709");
        assert_eq!(digest.to_hex_uppercase(), "DA39A3EE5E6B4B0D3255BFEF95601890AFD80709");

        let digest = new(Algorithm::SHA1).update("data").finalize().reset().digest();
        assert_eq!(digest.to_hex_lowercase(), "da39a3ee5e6b4b0d3255bfef95601890afd80709");
        assert_eq!(digest.to_hex_uppercase(), "DA39A3EE5E6B4B0D3255BFEF95601890AFD80709");
    }
    ...
    #[test]
    fn test_sha2_224() {
        assert_eq!(format!("{}", Algorithm::SHA2_224), "SHA-2 224");

        let digest = new(Algorithm::SHA2_224).digest();
        #[rustfmt::skip]
        ...
        assert_eq!(verify(Algorithm::SHA2_224, b"", digest), true);
        assert_eq!(format!("{digest:x}"), digest.to_hex_lowercase());
        assert_eq!(format!("{digest:X}"), digest.to_hex_uppercase());

        let digest = new(Algorithm::SHA2_224).update("data").digest();
        #[rustfmt::skip]
        ...
        assert_eq!(new(Algorithm::SHA2_224).update("data").finalize().digest(), digest);
        assert_eq!(Digest::from(sha2::sha224::hash(b"data")), digest);

        let digest = new(Algorithm::SHA2_224).update("data").reset().digest();
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_lowercase(), "d14a028c2a3a2bc9476102bb288234c415a2b01f828ea62ac5b3e42f");
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_uppercase(), "D14A028C2A3A2BC9476102BB288234C415A2B01F828EA62AC5B3E42F");

        let digest = new(Algorithm::SHA2_224).update("data").finalize().reset().digest();
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_lowercase(), "d14a028c2a3a2bc9476102bb288234c415a2b01f828ea62ac5b3e42f");
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_uppercase(), "D14A028C2A3A2BC9476102BB288234C415A2B01F828EA62AC5B3E42F");
    ...
    #[test]
    fn test_sha2_256() {
        assert_eq!(format!("{}", Algorithm::SHA2_256), "SHA-2 256");

        let digest = new(Algorithm::SHA2_256).digest();
        #[rustfmt::skip]
        ...
        assert_eq!(verify(Algorithm::SHA2_256, b"", digest), true);
        assert_eq!(format!("{digest:x}"), digest.to_hex_lowercase());
        assert_eq!(format!("{digest:X}"), digest.to_hex_uppercase());

        let digest = new(Algorithm::SHA2_256).update("data").digest();
        #[rustfmt::skip]
        ...
        assert_eq!(new(Algorithm::SHA2_256).update("data").finalize().digest(), digest);
        assert_eq!(Digest::from(sha2::sha256::hash(b"data")), digest);

        let digest = new(Algorithm::SHA2_256).update("data").reset().digest();
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_lowercase(), "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_uppercase(), "E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855");

        let digest = new(Algorithm::SHA2_256).update("data").finalize().reset().digest();
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_lowercase(), "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_uppercase(), "E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855");
    ...
    #[test]
    fn test_sha2_384() {
        assert_eq!(format!("{}", Algorithm::SHA2_384), "SHA-2 384");

        let digest = new(Algorithm::SHA2_384).digest();
        #[rustfmt::skip]
        ...
        assert_eq!(verify(Algorithm::SHA2_384, b"", digest), true);
        assert_eq!(format!("{digest:x}"), digest.to_hex_lowercase());
        assert_eq!(format!("{digest:X}"), digest.to_hex_uppercase());

        let digest = new(Algorithm::SHA2_384).update("data").digest();
        #[rustfmt::skip]
        ...
        assert_eq!(new(Algorithm::SHA2_384).update("data").finalize().digest(), digest);
        assert_eq!(Digest::from(sha2::sha384::hash(b"data")), digest);

        let digest = new(Algorithm::SHA2_384).update("data").reset().digest();
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_lowercase(), "38b060a751ac96384cd9327eb1b1e36a21fdb71114be07434c0cc7bf63f6e1da274edebfe76f65fbd51ad2f14898b95b");
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_uppercase(), "38B060A751AC96384CD9327EB1B1E36A21FDB71114BE07434C0CC7BF63F6E1DA274EDEBFE76F65FBD51AD2F14898B95B");

        let digest = new(Algorithm::SHA2_384).update("data").finalize().reset().digest();
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_lowercase(), "38b060a751ac96384cd9327eb1b1e36a21fdb71114be07434c0cc7bf63f6e1da274edebfe76f65fbd51ad2f14898b95b");
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_uppercase(), "38B060A751AC96384CD9327EB1B1E36A21FDB71114BE07434C0CC7BF63F6E1DA274EDEBFE76F65FBD51AD2F14898B95B");
    ...
    #[test]
    fn test_sha2_512() {
        assert_eq!(format!("{}", Algorithm::SHA2_512), "SHA-2 512");

        let digest = new(Algorithm::SHA2_512).digest();
        #[rustfmt::skip]
        ...
        assert_eq!(verify(Algorithm::SHA2_512, b"", digest), true);
        assert_eq!(format!("{digest:x}"), digest.to_hex_lowercase());
        assert_eq!(format!("{digest:X}"), digest.to_hex_uppercase());

        let digest = new(Algorithm::SHA2_512).update("data").digest();
        #[rustfmt::skip]
        ...
        assert_eq!(new(Algorithm::SHA2_512).update("data").finalize().digest(), digest);
        assert_eq!(Digest::from(sha2::sha512::hash(b"data")), digest);

        let digest = new(Algorithm::SHA2_512).update("data").reset().digest();
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_lowercase(), "cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e");
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_uppercase(), "CF83E1357EEFB8BDF1542850D66D8007D620E4050B5715DC83F4A921D36CE9CE47D0D13C5D85F2B0FF8318D2877EEC2F63B931BD47417A81A538327AF927DA3E");

        let digest = new(Algorithm::SHA2_512).update("data").finalize().reset().digest();
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_lowercase(), "cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e");
        #[rustfmt::skip]
        assert_eq!(digest.to_hex_uppercase(), "CF83E1357EEFB8BDF1542850D66D8007D620E4050B5715DC83F4A921D36CE9CE47D0D13C5D85F2B0FF8318D2877EEC2F63B931BD47417A81A538327AF927DA3E");
