...
//!
//! let digest = md5::hash("some data");
//! assert_eq!(md5::verify("some data", digest), true);
//! assert_eq!(md5::verify("SOME DATA", digest), false);
//! ```
//!
//! # Stream processing
//!
...
//!
//! # fn wrapper(path: PathBuf) -> io::Result<()> {
//! let digest_lowercase = md5::hash("some data");
//! let digest_uppercase = md5::hash("SOME DATA");
//!
//! let mut file = File::open(path)?;
//! let mut buffer = vec![0; 64];
//! let mut hash = md5::new();
...
///
/// let digest = md5::hash("data");
/// assert_eq!(md5::verify("data", digest), true);
/// assert_eq!(md5::verify("DATA", digest), false);
/// ```
#[cfg_attr(all(release, feature = "inline"), inline)]
#[must_use]
...
impl Update {
    ...
    /// use chksum_hash::md5;
    ///
    /// let hash = md5::new().update("data").finalize();
    /// let digest = hash.digest();
    /// assert_eq!(
    ///     digest.to_hex_lowercase(),
    ///     "8d777f385d3dfec8815d20f7496026dc"
